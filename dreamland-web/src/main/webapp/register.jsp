...
<script type="text/javascript">
    ...
    function checkPhone(){
        var phone = $("#phone").val();
        phone = phone.replace(/^\s+|\s+$/g,"");//去掉前后的空格
        if(phone == ""){
        ...
        if(!(/^1[3|4|5|8|7][0-9]\d{8}$/.test(phone))){
            ...
            // 手机号码判断正确以后，发送 AJAX 请求到后台，请求方式为 post ，请求 URL 为 /checkPhone，
            // 映射后台的某个 Controller 的方法，请求参数 data 是手机号，数据类型是 JSON 格式，
            // success 回调函数返回后台处理结果，是一个map（"message","success/fail"）放在 data 中，
            // 通过 data['message'] 取出放在 data 中的数据，
            // 如果是“success”，说明该手机号未被注册，则在 input 后的 span 标签中打“√”，并且如果 v=1，
            // 则将外部 div 减去30，动态缩短 div 高度，将变量 v 重置为0，并将 flag2 赋值为 true，表示手机号的 input 框状态已正确。
            $.ajax({
                type:'post',
                url:'/checkPhone',
                data: {"phone":phone},
